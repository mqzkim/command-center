...
async function upsert(p) {
  ...
  const res = await api('POST', `/api/kb/${KB}/documents/content`, {
    ...
    metadata: {
      ...
      stageKo: stageKo[p.toss?.stage || (p.legacy ? 'legacy' : 'n/a')] || (p.toss?.stage || (p.legacy ? 'legacy' : 'n/a')),
      nextCheck: p.toss?.nextCheck || 'n/a',
      blockersOpen: p.toss ? (p.toss.blockersOpen ?? p.toss.blockers?.length ?? 0) : 0,
