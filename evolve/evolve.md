...
3. **트렌드 검토**: `trends-latest.json`의 candidates(이미 wiki 카탈로그·seenRepos 중복 제거됨)에서 다음 기준으로 0~1개 선정:
   ...
   - **검토한 후보는 채택·기각 불문 `state.seenRepos`에 추가**한다 (trends.mjs는 seenRepos를 읽기만 한다 — 스캔이 후보를 소비하지 않도록 2026-08-23에 변경). 하루에 검토하는 후보는 상위 5~10건으로 제한해 원장을 점진적으로 채운다. 채택 항목은 `appliedImprovements`에 `{id, repo, date, mode}`로 기록.
   - 후보가 전부 부적합하면 (4)로. `CANDIDATES_EXHAUSTED`가 찍히면 §6으로 쿼리/별점 기준을 조정한다.
...
5. **검증**: 인라인 JS 문법(vm.Script), `node evolve/health.mjs` 재실행, http 200. 실패 상태로 커밋 금지.
   health는 **valid≠fresh** 원칙으로 판정한다(2026-08-26~): 산출물이 파싱된다는 것과 최신이라는 것은 다르다 — generatedAt 나이·gh 스냅샷 대비·refresh.log 빌드 exit를 함께 본다. 새 데이터 소스를 추가하면 같은 신선도 게이트를 health에 붙여라.
